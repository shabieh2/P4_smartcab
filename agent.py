...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=True):
        ...
        if testing:
            ...
            self.epsilon = math.e**(-0.01*self.trials)
            
            
            #if self.alpha>0.2:

                #self.alpha-=1/math.log(self.trials+2)
    ...
    def createQ(self, state):
        ...
        if self.learning and state not in self.Q.keys():
            self.Q[state]={None:0.00,'left':0.00,'right':0.00,'forward':0.00}
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if self.epsilon>random.random():
                ...
                maxQs=self.get_maxQ(state)

                actions = []
                for i in self.Q[state]:
                    if self.Q[state][i] == maxQs:
                        actions.append(i)
                if (len(actions) > 0):
